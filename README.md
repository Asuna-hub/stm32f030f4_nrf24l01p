...
# Настройка NRF24L01+
## Основная настройка 
Инициализировав всю необходимую периферию, подняв CSN и опустив CE, переходим к базовой настройке датчика, которая должна быть на всех устройствах одинаковой.
...
```
NRF24_Write_Reg(NRF24_REG_EN_AA, 1 << NRF24_ENAA_P0); // включаю автоподтверждение первой трубы
....
```
...
```
- Устанавлием количество повторных отправок и интервал между ними (работает только при влючённом автоподтверждении)
```
NRF24_Write_Reg(NRF24_REG_SETUP_RETR, NRF24_SETUP_RETR_ARD_250uS | NRF24_SETUP_RETR_ARC_15); // 15 повторов с интервалов 250 мкс
```
- Устанавливаем частотный канал
```
...
```
NRF24_Write_Reg_DMA(NRF24_REG_DPL, 1 << NRF24_DPL_P0); // DPL для 0 трубы
```
# Пример:
	void SPI1_Init(void){
